...
\hline&\\[-1em]
$\boldsymbol{A}$&It is possible to construct a signal flow graph with both input and output in normal order as shown in the figure below.\\[2pt]
\hline&\\[-1em]
$\boldsymbol{B}$& The number of butterflies in any stage is N/2\\[2pt]
\hline&\\[-1em]
$\boldsymbol{C}$&There are $\log_2 N$ stages and each stage consists of N/2 butterflies and each butterfly consists 2 nodes. So computation requires storage $N\log_2 N$ node data \\[2pt]
\hline&\\[-1em]
$\boldsymbol{D}$&Computation of butterfly requires only one complex multiplication and 2 complex addition.\\
\hline
\end{tabular}
\caption{Option explanations}
\label{table1}
\end{table}
\begin{figure}[!ht]
    \centering
    \includegraphics[width=\columnwidth]{SignalFlowGraph.png}
    \caption{Signal flow graph with both input and output in normal order for m=3\\
    Source : Oppenheim and Schafer}
    \label{fft}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=\columnwidth]{fft_butterfly.jpg}
    \caption{Signal flow graph representing butterflies for m=3}
    \label{fft}
\end{figure}
\end{document}
